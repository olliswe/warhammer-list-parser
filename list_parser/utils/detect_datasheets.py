...
def _strip_points_suffix(name: str) -> str:
    ...
    return re.sub(r"\(\s*\d+\s*(?:pts|points?)\s*\)$", "", name, flags=re.IGNORECASE).strip()
...
def _combined_best(query: str, choices: List[str], k=12):
    ...
    res = []
    for scorer in (fuzz.WRatio, fuzz.token_sort_ratio, fuzz.token_set_ratio, fuzz.partial_ratio):
...
def parse_datasheet_blocks(army_text: str) -> list[dict]:
    ...
    while i < n:
        ...
        if m:
            ...
            start = i
            j = i + 1
            while j < n and not POINTS_PAT.search(lines[j] or "") and lines[j].strip() != "":
                j += 1
...
def resolve_blocks_to_datasheets_prefer(
        blocks: List[Dict],
        catalog_all: List[Dict],   # each: {"datasheet_name","datasheet_id","faction_id", ...}
    ...
    for b in blocks:
        ...
        if not top:
            out.append({"datasheet_id": None, "datasheet_name": query, "entry_text": entry_text})
            continue
...
def extract_datasheet_entries_prefer(
        ...
        preferred_faction_ids: Iterable[str],
        hi: int = 92, lo: int = 70, prefer_bonus: int = 8, k: int = 12
) -> List[Dict]:
    blocks = parse_datasheet_blocks(army_text)
    return resolve_blocks_to_datasheets_prefer(
        blocks, catalog_all, preferred_faction_ids, hi=hi, lo=lo, prefer_bonus=prefer_bonus, k=k
    )

# ---------- helper to build a master catalog from factions.json ----------
def build_master_catalog() -> List[Dict]:
    ...
    master = []
    for fa in factions:
        for ds in fa.get("datasheets", []):
            master.append({
                "datasheet_name": ds["datasheet_name"],
                "datasheet_id": ds["datasheet_id"],
                "faction_id": fa["faction_id"],
            })
    return master
